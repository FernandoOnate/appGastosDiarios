...
if (isset($_POST['modificar'])) {
    // echo 'el boton modificar existe';
    ...
    if (isset($_POST['gasto'])) {
        ...
        if (!ValidarGastoVacio($_POST['gasto'])) {
            ...
            if (ValidarGastoNumerico($_POST['gasto'])) {
                ...
                // critico
                header('location:../../Vistas/Gastos/edit_gasto.php?cd=1100');
                die();
            }
        } else {
            // critico
            // echo 'el campo gasto no trajo un valor';
            header('location:../../Vistas/Gastos/edit_gasto.php?cd=1110');
            die();
        }
    } else {
        // critico
        // echo 'el campo gasto NO existe';
        header('location:../../Vistas/Gastos/edit_gasto.php?cd=1111');
        die();
    }
    ...
    if (isset($_POST['descripcion'])) {
        ...
        if (empty($_POST['descripcion'])) {
            ...
            header('location:../../Vistas/Gastos/edit_gasto.php?cd=1200');
            die();
        } else {
            if (!is_string($_POST['descripcion'])) {
                // critico
                // echo 'El campo descripcion no es un string';
                header('location:../../Vistas/Gastos/edit_gasto.php?cd=1210');
                die();
            } else {
                if (strlen($_POST['descripcion']) >= 105) {
                    //sobrepasa los 105 caracteres
                    header('location:../../Vistas/Gastos/edit_gasto.php?cd=1211');
                    die();
                } else {
    ...
    if (isset($_POST['fecha_creado'])) {
            ...
            if (strlen($_POST['fecha_creado']) <= 0 or strlen($_POST['fecha_creado']) > 19) {
                ...
                if (!is_string($_POST['fecha_creado'])) {
                    ...
                    try {
                        ...
                        header('location:../../Vistas/Gastos/edit_gasto.php?cd=1403');
                        die();
                    }
                }
            }
        ...
        //no existe la fecha
        header('location:../../Vistas/Gastos/edit_gasto.php?cd=1404');
        die();
    }
    ...
    if (isset($_POST['r'])) {
        if (!empty($_POST['r'])) {
            if (is_numeric($_POST['r'])) {
                ...
                //r es un string
                header('location:../../Vistas/Gastos/ver_gastos.php?cd=1502&edit=0');
                die();
            }
        } else {
            //el id del registro no vino
            header('location:../../Vistas/Gastos/ver_gastos.php?cd=1500&edit=0');
            die();
        }
    } else {
        //no existe ese registro en base de datos
        header('location:../../Vistas/Gastos/ver_gastos.php?cd=1501&edit=0');
        die();
    }
} else {
    // critico
    // echo 'el boton modificar no existe';
    header('location:../../Vistas/Gastos/edit_gasto.php?cd=1001');
    die();
}
...
if ($gasto and $descripcion and $fecha and $id_fila) {
    try {
        ...
        //throw $th;
        header('location:../../Vistas/Gastos/edit_gasto.php?cd=1310');
        die();
        // echo 'no agregado correctamente ' . $enlace->errno . ' ' . $enlace->error;
    }
} else {
    // echo 'Las variables son null';
    header('location:../../Vistas/Gastos/edit_gasto.php?cd=1300');
    die();
}
